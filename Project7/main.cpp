...
int main()
{
    ...
    Pig newGame;
    newGame.humanPlay(31); // Setting the human to roll huge amounts works!
    assert(newGame.isGameOver() == false); // Game isn't over until we end turn
    assert(newGame.determineGameOutcome() == Pig::GAMEOUTCOME::GAMENOTOVER);
    display( newGame, "", false); // We display to check
    newGame.humanEndTurn();
    assert(newGame.isGameOver() == true); // Now, the game is over
    assert(newGame.determineGameOutcome() == Pig::GAMEOUTCOME::HUMANWONGAME);
    display( newGame, "", false);
    
    Pig newGame2;
    newGame2.humanEndTurn();
    newGame2.computerPlay(-1); // Rolling -1 numbers works too
    newGame2.computerPlay(30);
    newGame2.computerEndTurn();
    display(newGame2,"", false);
    assert(newGame2.isGameOver() == false); // Game isn't over, since the computer only has 29 points
    assert(newGame2.determineGameOutcome() == Pig::GAMEOUTCOME::GAMENOTOVER);
    newGame2.humanEndTurn();
    assert(newGame2.computerPlay(-1)==-1); // We do this because a 1 resets the score to 0
    assert(newGame2.computerPlay(2)==2);
    newGame2.computerEndTurn();
    display(newGame2, "", false);
    assert(newGame2.isGameOver() == true); // Now, the game is over
    assert(newGame2.determineGameOutcome() == Pig::GAMEOUTCOME::COMPUTERWONGAME);
    display(newGame2,"", false);
    
    Pig newGame3;
    assert(newGame3.humanPlay(30)==30);
    assert(newGame3.humanPlay(1)==1);
    newGame3.humanEndTurn();
    assert(newGame3.isGameOver() == false); // Rolling the 1 resets the score, even if you had a winning score before
    assert(newGame3.determineGameOutcome() == Pig::GAMEOUTCOME::GAMENOTOVER);
        ...
        switch (action[0])
        {
            ...
            case 'R':
                ...
                if (human == 1)
                {
                    ...
                    do
                    {
                        if (handleComputerPlay( game2 ))
                            break;
                    } while (game2.computerKeepRolling());
                    game2.computerEndTurn();
                    display( game2, message, true );
                }
                break;
            case 'e':
            case 'E':
